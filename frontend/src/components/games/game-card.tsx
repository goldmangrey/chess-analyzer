...
export function GameCard({ game }: { game: ApiGameListItem }) {
  return (
    <article className="rounded-[1.5rem] bg-surface p-5 shadow-[var(--shadow-soft)]">
      ...
      <div className="mt-5 rounded-2xl bg-surface-muted p-4"><p className="text-sm">{game.opening_name ?? game.opening_code ?? "Дебют не определён"}</p>{game.opening_name && game.opening_code ? <p className="mt-1 text-xs font-bold text-forest-light">{game.opening_code}</p> : null}</div>
