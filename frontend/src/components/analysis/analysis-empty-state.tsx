...
const copy = {
  ...
  queued: ["Партия поставлена в очередь", "Worker начнёт анализ автоматически. Статус обновится без перезагрузки страницы."],
  processing: ["Stockfish анализирует партию", "Статус и готовый отчёт обновятся автоматически."],
  ...
  return (
    <BentoCard as="section" tone={status === "failed" ? "yellow" : "muted"} className="mt-6 p-7 text-center sm:p-12">
      ...
      <div className="mt-7 flex flex-wrap justify-center gap-3">
        {["queued", "processing", "analyzing"].includes(status) ? <Button variant="secondary" leftIcon={<RefreshCw size={16} />} onClick={() => router.refresh()}>Обновить данные</Button> : <AnalyzeGameButton gameId={gameId} status={status} />}
      </div>
    </BentoCard>
  );
}
