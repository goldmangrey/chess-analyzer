...
export function CriticalMomentsCard({ intelligence, selectedPly, onSelectPly }: { intelligence: GameIntelligenceResponse; selectedPly: number; onSelectPly: (ply: number) => void }) {
  ...
  const reviews = new Map((intelligence.move_reviews ?? []).map((review) => [review.ply, review.commentary] as const));
  ...
  return (
    <BentoCard as="section" className="h-full p-4 sm:p-5" aria-labelledby="critical-moments-title">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><p className="text-xs font-semibold text-forest-light">Ключевые события</p><h2 id="critical-moments-title" className="mt-1 text-lg font-semibold tracking-[-0.025em]">Критические моменты</h2></div>
      ...
      <ol className="mt-3 grid gap-2 sm:grid-cols-2">
        {moments.map((moment, index) => {
          ...
          const item = criticalMomentPresentation(moment, errors.get(moment.ply) ?? null, index + 1, reviews.get(moment.ply) ?? null);
          return <li key={`${moment.ply}-${moment.type}`}><button type="button" onClick={() => onSelectPly(moment.ply)} aria-current={active ? "step" : undefined} aria-label={`${item.rank}. ${item.moveLabel}. ${item.typeLabel}. ${item.severityLabel}`} className={`focus-ring group relative h-full min-h-32 w-full rounded-xl border p-3 text-left transition hover:bg-mint-surface ${active ? "border-forest bg-[color-mix(in_srgb,var(--forest)_6%,var(--surface))] shadow-[0_0_0_2px_color-mix(in_srgb,var(--forest)_12%,transparent)]" : "border-[var(--border-subtle)] bg-surface-muted"}`}>
            ...
            <p className="mt-3 text-sm font-semibold text-text-primary">{item.conciseReason}</p>
            {reviews.has(moment.ply) ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-text-secondary">{item.explanation}</p> : null}
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs"><span className="technical-number text-text-secondary">{item.evaluationBefore ?? "—"} → {item.evaluationAfter ?? "—"}</span>{item.phaseLabel ? <span className="font-semibold text-text-muted">{item.phaseLabel}</span> : null}</div>
