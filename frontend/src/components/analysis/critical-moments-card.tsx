...
export function CriticalMomentsCard({ intelligence, selectedPly, onSelectPly }: { intelligence: GameIntelligenceResponse; selectedPly: number; onSelectPly: (ply: number) => void }) {
  ...
  return (
    <BentoCard as="section" className="mt-6 p-5 sm:p-6" aria-labelledby="critical-moments-title">
      ...
      <ol className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {moments.map((moment, index) => {
          ...
          return <li key={`${moment.ply}-${moment.type}`}><button type="button" onClick={() => onSelectPly(moment.ply)} aria-current={active ? "step" : undefined} aria-label={`${item.rank}. ${item.moveLabel}. ${item.typeLabel}. ${item.severityLabel}`} className={`focus-ring group relative h-full min-h-44 w-full rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md ${active ? "border-forest bg-[color-mix(in_srgb,var(--forest)_7%,var(--surface))] shadow-[0_0_0_2px_color-mix(in_srgb,var(--forest)_15%,transparent)]" : "border-[var(--border-subtle)] bg-surface-muted"}`}>
