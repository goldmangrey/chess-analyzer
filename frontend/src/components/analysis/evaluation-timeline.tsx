...
function TimelineDot({ cx, cy, payload }: { cx?: number; cy?: number; payload?: EvaluationTimelinePoint }) {
  ...
  const errorRadius = payload.classification === "blunder" ? 5 : payload.classification === "mistake" ? 4 : payload.classification === "inaccuracy" ? 3.25 : 1.75;
  ...
  return <g opacity={payload.classification === "normal" && !payload.isSelected ? 0.58 : 1}>{payload.isSelected ? <circle cx={cx} cy={cy} r={9} fill="var(--surface)" stroke="var(--forest)" strokeWidth={2.25} /> : null}<circle cx={cx} cy={cy} r={radius} fill={fill} stroke={payload.isCritical ? "var(--surface)" : "var(--forest-light)"} strokeWidth={payload.isCritical ? 2.5 : 1.25} /></g>;
}
