...
for secret in DATABASE_PASSWORD CHESSCOM_USER_AGENT ANALYSIS_WORKER_SHARED_SECRET SCHEDULED_SYNC_SHARED_SECRET; do gcloud secrets describe "$secret" --project "$GCP_PROJECT_ID" >/dev/null 2>&1 && echo "✓ secret $secret exists" || echo "⚠ secret $secret is not created"; done
...
echo "Scheduler: $SCHEDULER_JOB ($SCHEDULER_SCHEDULE, $SCHEDULER_TIME_ZONE)"
