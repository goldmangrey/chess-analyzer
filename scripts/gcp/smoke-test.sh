...
if [[ -n "${SYNC_SERVICE_URL:-}" ]] && $APPLY; then code="$(curl -sS -o /dev/null -w '%{http_code}' -X POST "$SYNC_SERVICE_URL/internal/sync/chess-com" -H 'Content-Type: application/json' -d '{"schema_version":1}')"; [[ "$code" == 401 || "$code" == 403 ]] || { echo "Sync service unauthenticated check returned $code" >&2; exit 1; }; fi
