...
for name in DATABASE_PASSWORD CHESSCOM_USER_AGENT ANALYSIS_WORKER_SHARED_SECRET SCHEDULED_SYNC_SHARED_SECRET; do
  ...
  if $APPLY && { $created || [[ "${ROTATE_SECRETS:-false}" == true ]]; }; then
    ...
    echo "Secret $name exists; version unchanged"
  elif $created || [[ "${ROTATE_SECRETS:-false}" == true ]]; then
    echo "Would add a secret version to $name from environment (value hidden)"
  else
    echo "Secret $name exists; version would remain unchanged"
  fi
done
